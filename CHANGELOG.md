...
- Milestone 10C: Installer / Setup Improvements — added read-only first-run setup verifier (`scripts/setup-check.ps1`) with tool, dependency, npm script, port, and optional live endpoint checks; strengthened `doctor.ps1`; made `start-backend.ps1` fail early on missing dependencies or occupied non-CareerOS port 8000; updated startup docs and script coverage
- Milestone 10C-live-test: Released local developer-preview MVP for live test; verified setup-check, doctor, full backend/desktop/extension suites, full extension assertion inventory, Ruff, dependency/privacy gates, and updated release readiness checks
- GitOps policy update: documented Git/GitHub as the source of truth for source, docs, tests, release manifests, workflow files, and reviewed non-sensitive fixtures; added privacy boundaries for local databases, secrets, generated files, private resumes, private application records, and unsanitized reference data
