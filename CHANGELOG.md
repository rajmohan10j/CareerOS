...
- Milestone 10A-FIX-4: Resume Upload / Import MVP — Resumes page now shows Upload Resume (file picker for .txt/.md, PDF/DOC/DOCX warning), Paste Resume Text (textarea + title), Create Blank Resume buttons. Download .txt and .md buttons with Blob/object URL (no cloud). Folder import limitation message. Metadata display on content view. ResumeCreate schema gains content field. ResumeService.create() passes content through. API client adds fetchResume, importResumeText, uploadResumeFile, uploadResumeTextFile, downloadResumeText. Empty state replaced with actionable buttons. Desktop tests: 258 passed. Backend tests: 33 resume tests passed (1 new). Ruff clean. No paid APIs, no telemetry.
